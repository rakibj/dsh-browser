...
- The bridge connects at extension startup and reconnects without an open side panel. You can chat in the Harness web UI while using another tab or application. Switching tabs leaves browser tools bound to their existing target.
- `browser_open_tab` creates an inactive tab in a blue **DSH** group for that session. When the agent becomes idle after completing, cancelling, or failing its work, the bridge closes only tabs that session opened; Chrome removes the empty group. Existing ungrouped target tabs join DSH temporarily and are ungrouped on completion; they are never closed automatically. Existing user groups are left intact. Temporary tabs are tracked across background-worker restarts, and completion during a transport disconnect is delivered on reconnect. Restarting the Harness process itself does not recover missed completion events.
- Reload the unpacked extension and restart Harness after updating both halves of the bridge. Background connection does not bypass approvals: requests that need permission still wait for the side panel or an approval notification.
